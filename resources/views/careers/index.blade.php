...
<head>
    ...
    <style>
        ...
        .brand-ring {
            ...
            box-shadow: 0 10px 25px -5px rgba(180, 134, 60, 0.15);
        }
        ...
        .brand-card:hover {
            ...
            box-shadow: 0 18px 34px -18px rgba(32, 26, 18, 0.25);
        }
        .brand-logo-slot {
            height: 44px;
            display: flex;
        ...
        .spec-card:hover {
            ...
            box-shadow: 0 20px 40px -15px rgba(180, 134, 60, 0.15);
        ...
        /* ---------- HTML Content Styles (Modal) ---------- */
        .quill-content { color: var(--slate); font-size: 0.9rem; line-height: 1.7; }
        .quill-content h1, .quill-content h2, .quill-content h3 { font-family: 'Cormorant Garamond', serif; color: var(--ink); font-weight: 700; margin-bottom: 0.5rem; margin-top: 1rem; }
        ...
        .focus-ring:focus {
            ...
            border-color: var(--gold);
            background: var(--paper-raised);
            box-shadow: 0 0 0 3px rgba(180, 134, 60, 0.15);
        }
        ...
        .contact-icon {
            ...
            border: 1px solid rgba(180, 134, 60, 0.3);
            display: flex;
            align-items: center;
            justify-content: center;
            flex-shrink: 0;
...
<body x-data="{ modalOpen: false, selectedJob: '', selectedDesc: '' }">
    ...
    <header class="site-header">
        <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
            <a href="/" class="flex items-center gap-3 group">
                <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" class="group-hover:scale-105 transition-transform">
                ...
                <div class="leading-tight">
                    <p class="font-display font-semibold text-xl tracking-[0.08em]" style="color: var(--ink);">TF MOTORS</p>
                    <p class="font-mono text-[10px] tracking-[0.25em] uppercase" style="color: var(--gold);">Careers</p>
                </div>
            </a>
            <a href="mailto:hello@tfmotors.com" class="hidden sm:flex items-center gap-2 text-sm font-semibold hover:text-[#b4863c] transition-colors" style="color: var(--slate);">
                hello@tfmotors.com
            </a>
        </div>
    </header>
    ...
    <section class="hero-section py-20 md:py-28">
        <div class="max-w-4xl mx-auto px-6 text-center">
            ...
            <div class="inline-flex items-center gap-2 px-3 py-1 mb-6 rounded-full border bg-white shadow-sm" style="border-color: var(--line);">
                <span class="w-1.5 h-1.5 rounded-full animate-pulse" style="background: var(--gold);"></span>
                <p class="font-mono text-[10px] tracking-[0.2em] uppercase font-bold" style="color: var(--gold-deep);">
    ...
    <section class="max-w-6xl mx-auto px-6 pb-16">
        <div class="brand-frame p-8 md:p-10">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-6">
                @php
                    $brands = [
                        ['name' => 'MG', 'blurb' => "Established in 1924, Britain's first automobile brand with a long heritage over 100 years."],
                        ['name' => 'MAXUS', 'blurb' => 'We bring you premium and smart vehicles, combining advanced technology and luxury.'],
                        ['name' => 'ROYAL ENFIELD', 'blurb' => 'Royal Enfield is an iconic motorcycle brand made for the modern classic rider.'],
                        ['name' => 'PEUGEOT', 'blurb' => 'Peugeot Motocycles, a French icon since 1898 delivering elegance on two wheels.'],
                        ['name' => 'LEAPMOTOR', 'blurb' => 'Intelligent EV brand focusing on advanced technology and sustainable journeys.'],
                    ];
                @endphp
                ...
                <div class="brand-card text-center group">
                    <div class="brand-logo-slot justify-center mb-3">
                        <span class="font-display font-semibold text-lg tracking-wide group-hover:text-[#b4863c] transition-colors" style="color: var(--ink);">{{ $brand['name'] }}</span>
                    </div>
                    <div class="brand-photo-slot mb-3 group-hover:border-[#b4863c] transition-colors">
    ...
    <section class="max-w-6xl mx-auto px-6 pb-20">
        <div class="flex items-center justify-center gap-3 mb-10">
            <span class="divider-dot"></span>
            <h2 class="font-display font-semibold text-3xl tracking-wide" style="color: var(--ink);">Open Roles</h2>
            <span class="divider-dot"></span>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="spec-card p-8 flex flex-col justify-between group">
                <div>
                    <div class="flex justify-between items-start mb-3">
                        ...
                        @if($job->is_urgent)
                            <span class="flex h-2.5 w-2.5 relative">
                              ...
                              <span class="relative inline-flex rounded-full h-2.5 w-2.5 bg-red-500" title="Urgent"></span>
                            </span>
                        @endif
                    </div>
                    
                    <h2 class="font-display font-semibold text-3xl mb-5 leading-tight group-hover:text-[#b4863c] transition-colors" style="color: var(--ink);">
                        {{ $job->title }}
                    </h2>
                    ...
                    {{-- ⚡️ Clean Description with strip_tags --}}
                    <p class="text-sm leading-relaxed mt-4 line-clamp-3" style="color: var(--slate);">
                ...
                <button @click="modalOpen = true; selectedJob = {{ Js::from($job->title) }}; selectedDesc = {{ Js::from($job->description) }}"
                        class="btn-shine mt-6 w-full py-3.5 rounded-lg text-sm font-semibold text-white tracking-wide shadow-md"
                        style="background: var(--gold);">
                    View Details & Apply
                </button>
            </div>
            @empty
            <div class="md:col-span-2 text-center py-24 rounded-2xl spec-card border-dashed">
                <p class="font-display font-semibold text-2xl mb-2" style="color: var(--ink);">No open roles right now.</p>
    ...
    <footer class="site-footer">
        <div class="max-w-6xl mx-auto px-6 py-16 relative">
            <div class="flex items-center gap-3 mb-10">
                ...
                <p class="font-mono text-xs tracking-[0.25em] uppercase font-bold" style="color: var(--gold);">Get In Touch</p>
            </div>
                ...
                <div class="contact-card">
                    ...
                    <div>
                        ...
                        <p class="contact-value">
                            ...
                            Sangkat Teuk Thla, Khan Sen Sok, Phnom Penh, 120802
                        </p>
                        <a href="https://www.bing.com/maps/default.aspx?v=2&amp;pc=FACEBK&amp;mid=8100&amp;where1=No.%20174-175%2C%20California%20Social%20House%2C%20Russian%20Federation%20Blvd%2C%20Sangkat%20Teuk%20Thla%2C%20Khan%20Sen%20Sok%2C%20Phnom%20Penh%2C%20Cambodia%2C%20120802&amp;FORM=FBKPL1"
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" x-cloak>
        ...
        <div x-show="modalOpen" 
             ...
             class="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl flex flex-col md:flex-row max-h-[90vh] overflow-hidden"
             style="border-top: 4px solid var(--gold);">
            
            <div class="w-full md:w-1/2 p-8 md:p-10 overflow-y-auto custom-scrollbar border-b md:border-b-0 md:border-r" style="background: var(--paper); border-color: var(--line);">
                <p class="font-mono text-[10px] tracking-[0.2em] uppercase mb-3 font-bold" style="color: var(--gold-deep);">Role Details</p>
                <h2 class="font-display font-bold text-3xl md:text-4xl mb-6 leading-tight" style="color: var(--ink);" x-text="selectedJob"></h2>
                
                {{-- ⚡️ HTML Rendering with Quill Styling --}}
                <div class="quill-content" x-html="selectedDesc"></div>
            </div>

            <div class="w-full md:w-1/2 p-8 md:p-10 overflow-y-auto custom-scrollbar relative bg-white">
                <button @click="modalOpen = false" class="absolute top-6 right-6 p-2 rounded-full hover:bg-slate-100 transition-colors focus:outline-none" style="color: var(--slate);">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>

                <h3 class="font-display font-semibold text-2xl mb-2 mt-4" style="color: var(--ink);">Submit Application</h3>
                <p class="text-sm mb-8" style="color: var(--slate);">Please provide your details and attach your resume.</p>

                {{-- ⚡️ ID Added for SweetAlert, x-data for file name preview --}}
                <form id="applyForm" action="{{ route('careers.apply') }}" method="POST" enctype="multipart/form-data" class="space-y-5" x-data="{ fileName: '' }">
                    ...
                    <div>
                        <label class="block font-mono text-[10px] tracking-[0.1em] uppercase font-bold mb-2" style="color: var(--slate);">Full Name</label>
                        <input type="text" name="name" placeholder="John Doe" class="focus-ring w-full p-3.5 text-sm rounded-lg" required>
                    </div>
                    
                    <div>
                        <label class="block font-mono text-[10px] tracking-[0.1em] uppercase font-bold mb-2" style="color: var(--slate);">Email Address</label>
                        <input type="email" name="email" placeholder="john@example.com" class="focus-ring w-full p-3.5 text-sm rounded-lg" required>
                    </div>

                    <div>
                        <label class="block font-mono text-[10px] tracking-[0.1em] uppercase font-bold mb-2" style="color: var(--slate);">Resume (PDF)</label>
                        <label class="flex flex-col items-center justify-center w-full p-6 rounded-lg border-2 border-dashed cursor-pointer transition-colors group"
                               style="border-color: var(--gold-soft); background: var(--paper);">
                            <svg class="w-6 h-6 mb-2 transition-transform group-hover:scale-110" style="color: var(--gold-deep);" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                            {{-- ⚡️ Update Text Dynamically --}}
                            <span class="text-sm font-semibold text-center px-4" style="color: var(--ink);" x-text="fileName ? fileName : 'Upload Resume'"></span>
                            <span class="text-[11px] mt-1" style="color: var(--slate);" x-show="!fileName">PDF up to 5MB</span>
                            <input type="file" name="resume" class="hidden" required accept=".pdf" @change="fileName = $event.target.files[0].name">
                        </label>
                    </div>

                    <button type="submit" class="btn-shine w-full mt-4 py-4 rounded-lg font-semibold text-sm tracking-wide flex items-center justify-center gap-2 text-white" style="background: var(--ink);">
                        Send Application
