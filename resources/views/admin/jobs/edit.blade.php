...
@section('title', 'កែប្រែការងារ | TF Admin')
@section('header_title', 'កែប្រែការងារ (Edit Job)')
...
<style>
    ...
    .form-card::before {
        ...
        background: linear-gradient(90deg, #3b82f6, #6366f1); /* ដូរពណ៌បន្ទាត់លើឱ្យខុសពី Create បន្តិច */
    }
    ...
    .form-input.field-error { border-color: #fda4af; }
    .form-input.field-error:focus { border-color: #f43f5e; box-shadow: 0 0 0 4px rgba(244, 63, 94, 0.12); }
    ...
    /* ⚡️ ---------- ពណ៌ និងក្បាច់រចនាសម្រាប់ Quill Editor ---------- */
    .editor-wrapper {
        border-radius: 0.85rem;
        background: #f8fafc;
        border: 1px solid #e2e8f0;
...
<div class="page-shell">
    ...
    <div class="hero-banner animate-card">
        <div class="hero-content">
            <span class="hero-badge"><span class="dot"></span> កែប្រែព័ត៌មាន</span>
            <h1 class="hero-title">កែប្រែការងារ (Edit Job)</h1>
            <p class="hero-subtitle">ធ្វើបច្ចុប្បន្នភាពព័ត៌មានលម្អិតសម្រាប់ការងារ: <strong class="text-white">{{ $job->title }}</strong></p>
        </div>
    ...
    <form id="job-form" action="{{ route('admin.jobs.update', $job->id) }}" method="POST" class="form-card animate-card">
        @csrf
        @method('PUT')
        ...
        <div class="field-grid mb-8">
            <div class="form-group md:col-span-2" style="grid-column: 1 / -1;">
                ...
                <div class="input-shell">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $job->title) }}" placeholder="ឧ. Senior Software Developer"
            ...
            <div class="form-group">
                ...
                <div class="input-shell">
                    ...
                    <input type="text" name="location" id="location" value="{{ old('location', $job->location) }}" placeholder="ឧ. Phnom Penh" class="form-input" required>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="input-shell">
                    ...
                    <input type="text" name="salary_range" id="salary_range" value="{{ old('salary_range', $job->salary_range) }}" placeholder="ឧ. $500 - $800 ឬ Competitive" class="form-input">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="input-shell">
                    ...
                    <select name="status" id="status" class="form-input appearance-none cursor-pointer">
                        <option value="Open" {{ old('status', $job->status) == 'Open' ? 'selected' : '' }}>🟢 Open (កំពុងជ្រើសរើស)</option>
                        <option value="Closed" {{ old('status', $job->status) == 'Closed' ? 'selected' : '' }}>🔴 Closed (បិទការជ្រើសរើស)</option>
                    </select>
                    <svg class="w-4 h-4 absolute right-4 text-slate-400 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                </div>
            </div>
        ...
        <div class="form-group">
            ...
            <input type="hidden" name="description" id="hidden_description" value="{{ old('description', $job->description) }}">
